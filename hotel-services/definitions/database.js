...
if (CONF.database || process.env.DATABASE_CONNECTION) {
    ...
    require('querybuildermysql2').init('default', conn);

    console.log('---> DBMS: Initializing with ' + conn.split('@')[1]); // Logs host only for safety
} else {
...
// ON('boot', function() {
//     const conn = CONF.database || process.env.DATABASE_CONNECTION;
//     console.log('---> DBMS: Initialized with:', conn);
